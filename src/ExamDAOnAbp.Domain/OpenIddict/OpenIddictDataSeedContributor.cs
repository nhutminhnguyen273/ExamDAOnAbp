...
namespace ExamDAOnAbp.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("ExamDAOnAbp") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "ExamDAOnAbp", DisplayName = "ExamDAOnAbp API", Resources = { "ExamDAOnAbp" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["ExamDAOnAbp_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["ExamDAOnAbp_Web:RootUrl"]!.EnsureEndsWith('/');

            /* ExamDAOnAbp_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["ExamDAOnAbp_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["ExamDAOnAbp_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["ExamDAOnAbp_Swagger:RootUrl"]?.TrimEnd('/');
